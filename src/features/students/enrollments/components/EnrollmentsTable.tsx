...
const O: { label: string; value: EnrollmentOrderingT }[] = [
  { label: "Estudiante (A-Z)", value: "student_name" },
  { label: "Estudiante (Z-A)", value: "-student_name" },
  { label: "Curso (A-Z)", value: "section_name" },
  { label: "Curso (Z-A)", value: "-section_name" },
  { label: "Fecha (asc)", value: "enrollment_date" },
  { label: "Fecha (desc)", value: "-enrollment_date" },
  { label: "Estado", value: "enrollment_status" },
  { label: "Estado (desc)", value: "-enrollment_status" },
];

type Props = {
  enrollments: EnrollmentT[];
  isLoading: boolean;
  loadEnrollments: (p?: EnrollmentListParamsT) => void;
  onEdit: (e: EnrollmentT) => void;
  onView: (e: EnrollmentT) => void;
  onDelete: (e: EnrollmentT) => void;
};
...
export const EnrollmentsTable = ({
  ...
  const [ordering, setOrdering] = useState<EnrollmentOrderingT>("student_name");
  const [hasSearched, setHasSearched] = useState(false);
  const [sectionFilter, setSectionFilter] = useState<number | 0>(0);
  const [statusFilter, setStatusFilter] = useState<string>("");
  const [studentFilter, setStudentFilter] = useState<number | 0>(0);
  const { sectionOptions, studentOptions, statusOptions } =
    useEnrollmentFilterCatalogs();
  ...
  const fetchData = useCallback(
    (o?: {
      ...
      loadEnrollments({
        ...
        filters: o?.filters ?? buildFilters(),
      });
    },
  ...
  const hOrder = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      const n = e.target.value as EnrollmentOrderingT;
      setOrdering(n);
      setPage(1);
  ...
  const hSection = useCallback((o: SelectOptionT) => {
    setSectionFilter(Number(o.value) || 0);
    setPage(1);
  }, []);
  const hStatus = useCallback((o: SelectOptionT) => {
    setStatusFilter(o.value as string);
    setPage(1);
  }, []);
  const hStudent = useCallback((o: SelectOptionT) => {
    setStudentFilter(Number(o.value) || 0);
    setPage(1);
  }, []);
  useEffect(() => {
    fetchData({ page: 1 });
  }, [sectionFilter, statusFilter, studentFilter]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const cols: TableColumnProps<EnrollmentT>[] = [
    ...
    { key: "section_name", label: "Curso", className: tableColumnsClassname },
    {
      ...
      render: (e) => <span>{formatDate(e.enrollment_date)}</span>,
    },
    {
      ...
      className: tableColumnsClassname,
      render: (e) => {
        const cfg = getStatusConfig(e.enrollment_status);
        return (
          <Badge variant={cfg.badge as "default" | "secondary" | "outline"}>
            {cfg.label}
          </Badge>
        );
      },
    ...
    <div className="overflow-visible rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-slate-50/50 px-4 py-3">
        <SearchInput
          ...
          placeholder="Filtrar matrículas..."
        />
        <CustomSelect
          name="filter-section"
          label=""
      ...
      <CustomTable<EnrollmentT>
        data={enrollments}
        columns={cols}
        isLoading={isLoading && enrollments.length === 0}
        emptyMessage={
          hasSearched
            ? "No se encontraron matrículas con los filtros"
