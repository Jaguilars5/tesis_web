...
interface EarlyAlertDeleteModalProps {
  isOpen: boolean;
...
export const EarlyAlertDeleteModal: React.FC<EarlyAlertDeleteModalProps> = ({
        ...
        <div className="p-5">
            ...
            <div className="flex flex-col items-center gap-4 py-2 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-slate-900">
                  {message ?? "¿Está seguro de desactivar esta alerta?"}
                </p>
                <p className="mt-2 text-sm text-slate-600">{entity.enrollment_name}</p>
          ...
          {phase === "deactivating" && (
            <div className="flex flex-col items-center gap-3 py-4 text-center">
              ...
              <p className="text-sm text-slate-600">Desactivando alerta...</p>
            </div>
          )}
          ...
          {phase === "done" && (
            <div className="flex flex-col items-center gap-4 py-2 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-slate-900">Alerta desactivada exitosamente</p>
          ...
          {phase === "error" && (
            <div className="flex flex-col items-center gap-4 py-2 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-red-700">{errorMsg ?? "Error al desactivar la alerta"}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-200 px-5 py-4">
          ...
          {(phase === "done" || phase === "error") && (
            <button type="button" onClick={onClose}
              className="inline-flex items-center justify-center rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-primary-hover">Cerrar</button>
          )}
          {phase === "probing" && (
            <button type="button" onClick={onClose} disabled={isBusy}
              className="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">Cancelar</button>
