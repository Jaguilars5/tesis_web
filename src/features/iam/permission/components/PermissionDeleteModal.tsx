...
import type { PermissionDeleteParamsT } from "../permission.types";
import type { PermissionT } from "../permission.types";

interface Props {
  isOpen: boolean;
  entity: PermissionT | null;
  onClose: () => void;
  onSoftDelete: (
    params: PermissionDeleteParamsT,
  ) => Promise<SoftDeleteResponseT>;
}

export const PermissionDeleteModal: React.FC<Props> = ({
  isOpen,
  entity,
  onClose,
  onSoftDelete,
        ...
        <div className="p-5">
          ...
          {phase === "deactivating" && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-sm text-slate-500">Eliminando permiso...</p>
            </div>
          )}
              ...
              <div>
                ...
                {deactivatedRecords !== null && deactivatedRecords > 0 && (
                  <p className="mt-1 text-xs text-slate-500">
                    {deactivatedRecords} registro(s) afectado(s) en cascada
                  </p>
                )}
              </div>
            </div>
          ...
          {phase === "error" && (
            <div className="flex flex-col items-center gap-3 py-4 text-center">
              ...
              <p className="text-sm text-red-600">
                {errorMsg ?? "Error al eliminar el permiso"}
              </p>
            </div>
          )}
        </div>
