...
import type { UserDeleteParamsT } from "../users.types";
...
interface Props {
  ...
  entity: UserT | null;
  onClose: () => void;
  onSoftDelete: (params: UserDeleteParamsT) => Promise<SoftDeleteResponseT>;
}
...
export const UsersDeleteModal: React.FC<Props> = ({
  ...
  const {
    phase, message, affectedRecords, deactivatedRecords, errorMsg, confirm,
  } = useSoftDeleteFlow({
    isOpen,
    id: entity?.id ?? null,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        ...
        <div className="p-5">
          ...
          {phase === "confirm" && (
            <>
              <div className="flex flex-col items-center gap-4 py-2 text-center">
                ...
                <div>
                  <p className="text-sm text-slate-600">¿Está seguro de eliminar el usuario <span className="font-semibold text-slate-900">{entity.names} {entity.last_names}</span>?</p>
              ...
              {message && (
                <div className="mt-3 rounded-lg bg-amber-50 p-3 text-sm text-amber-700">
                  ...
                  {affectedRecords !== null && <p className="mt-1 text-xs">Registros afectados: <strong>{affectedRecords}</strong></p>}
                </div>
              )}
            </>
          )}
          ...
          {phase === "deactivating" && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-sm text-slate-500">Eliminando usuario...</p>
            </div>
          )}
          ...
          {phase === "done" && (
            <div className="flex flex-col items-center gap-3 py-4 text-center">
              ...
              <p className="text-sm font-medium text-slate-900">Usuario eliminado exitosamente</p>
              {deactivatedRecords !== null && deactivatedRecords > 0 && (
                <p className="mt-1 text-xs text-slate-500">{deactivatedRecords} registro(s) afectado(s) en cascada</p>
              )}
            </div>
          )}
          ...
          {phase === "error" && (
            <div className="flex flex-col items-center gap-3 py-4 text-center">
              ...
              <p className="text-sm text-red-600">{errorMsg ?? "Error al eliminar el usuario"}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-200 px-5 py-4">
          {phase === "confirm" && (
            <>
              <button type="button" onClick={onClose} disabled={isBusy} className="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">
                Cancelar
              </button>
