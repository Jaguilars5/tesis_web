...
export const conductIncidentSchema = Yup.object({
  incident_type: Yup.number().nullable().required("El tipo de incidente es requerido"),
  severity: Yup.number().nullable().required("La severidad es requerida"),
  academic_period: Yup.number().nullable().required("El período académico es requerido"),
