...
import { useIncidentTypeOptions, useSeverityOptions, useEnrollmentOptions, useAcademicPeriodOptions } from "./hooks/useConductIncidentOptions";
...
import { ConductIncidentDeleteModal } from "./components/ConductIncidentDeleteModal";
...
export default function ConductIncidentsPage() {
  ...
  const canCreate = hasPermission(permissions, CONDUCT_INCIDENT_PERMISSIONS.CREATE);
  const canEdit = hasPermission(permissions, CONDUCT_INCIDENT_PERMISSIONS.UPDATE);
  const canDelete = hasPermission(permissions, CONDUCT_INCIDENT_PERMISSIONS.DELETE);
  ...
  const [deletingItem, setDeletingItem] = useState<ConductIncidentT | null>(null);
  ...
  const openDeleteModal = useCallback((incident: ConductIncidentT) => { setDeletingItem(incident); setIsDeleteOpen(true); }, []);
  ...
  return (
    <div className="space-y-4">
      ...
      <ConductIncidentTable conductIncidents={items} isLoading={isLoading} loadConductIncidents={loadItems} onEdit={openModal} onView={openViewModal} onDelete={openDeleteModal} canEdit={canEdit} canDelete={canDelete} />

      <ConductIncidentFormModal key={editingItem?.id ?? "create"} isOpen={isOpen} onClose={closeModal} isEdit={isEdit} editingIncident={editingItem} onSubmit={handleSubmit} submitErrors={submitErrors}
      ...
      <ConductIncidentDeleteModal isOpen={isDeleteOpen} conductIncident={deletingItem} onClose={closeDeleteModal} onSoftDelete={deleteItem} />
    </div>
  );
}
