...
function useOptions(loader: () => Promise<Option[]>, deps: unknown[] = []) {
  ...
  useEffect(() => {
    ...
    loader()
      .then((items) => {
        if (!cancelled) dispatch({ type: "success", options: items });
      })
      .catch(() => {
...
export const useEnrollmentOptions = () => {
  const { options, loading } = useOptions(() =>
    enrollmentService.list({ page: 1, pageSize: 100 }).then((items) =>
      items.map((i) => ({
        label: i.student_name ?? `Matrícula ${i.id}`,
        value: String(i.id),
      })),
    ),
  );
...
export const useAcademicPeriodOptions = () => {
  const { options, loading } = useOptions(() =>
    academicPeriodService
      .list({ page: 1, pageSize: 100 })
      .then((result) =>
        result.items.map((i) => ({ label: i.name, value: String(i.id) })),
      ),
  );
