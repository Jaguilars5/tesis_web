...
import { useBehaviorEvaluationController } from "./hooks/useBehaviorEvaluationController";
import { useBehaviorEvaluationForm } from "./hooks/useBehaviorEvaluationForm";
...
import { BEHAVIOR_EVALUATION_PERMISSIONS } from "./behavior-evaluation.constants";
...
export default function BehaviorEvaluationPage() {
  ...
  const { isOpen, editingItem, submitErrors, openModal, closeModal, handleSubmit } =
    useBehaviorEvaluationForm({ update: updateItem });
  ...
  const handleCalculate = useCallback(async () => {
    ...
    await calculateItem({ enrollment_id: calculateEnrollmentId, academic_period_id: calculatePeriodId });
    setIsCalculating(false);
    closeCalculateModal();
  }, [calculateEnrollmentId, calculatePeriodId, calculateItem, closeCalculateModal]);
  ...
  return (
    <div className="space-y-4">
      ...
      <BehaviorEvaluationFormModal key={editingItem?.id ?? "edit"} isOpen={isOpen} onClose={closeModal} editingEvaluation={editingItem} onSubmit={handleSubmit} submitErrors={submitErrors} />
