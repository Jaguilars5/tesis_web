...
const behaviorEvaluationSlice = createSlice({
  ...
  reducers: {
    ...
    loadSuccess(state, action: PayloadAction<BehaviorEvaluationT[]>) { state.items = action.payload; state.status = "succeeded"; },
...
export const {
  ...
  mutationError, clearBehaviorEvaluationError,
