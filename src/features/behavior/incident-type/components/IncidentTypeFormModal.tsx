...
import type { SubmitErrorState } from "../incident-type.controller";
import type { IncidentTypeFormValues, IncidentTypeT } from "../incident-type.types";

const getFieldLabel = (f: string): string => { const labels: Record<string, string> = { code: "Código", name: "Nombre", description: "Descripción", non_field_errors: "Error general" }; return labels[f] || f; };

interface Props { isOpen: boolean; onClose: () => void; isEdit: boolean; editingIncidentType: IncidentTypeT | null; onSubmit: (values: IncidentTypeFormValues) => Promise<void>; submitErrors: SubmitErrorState; }

export const IncidentTypeFormModal = ({ isOpen, onClose, isEdit, editingIncidentType, onSubmit, submitErrors }: Props) => {
  const getInitialValues = (): IncidentTypeFormValues => {
    if (editingIncidentType) return { code: editingIncidentType.code, name: editingIncidentType.name, description: editingIncidentType.description, is_active: editingIncidentType.is_active };
    return { code: "", name: "", description: "", is_active: true };
  };
  const formik = useFormik<IncidentTypeFormValues>({ initialValues: getInitialValues(), validationSchema: incidentTypeSchema, enableReinitialize: true, onSubmit });
  useEffect(() => { if (isOpen && editingIncidentType) formik.setValues(getInitialValues()); }, [isOpen, editingIncidentType]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div><h3 className="text-lg font-semibold text-slate-900">{isEdit ? "Editar Tipo" : "Nuevo Tipo"}</h3><p className="mt-0.5 text-sm text-slate-500">Configure el tipo de incidente de conducta</p></div>
