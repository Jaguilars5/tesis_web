...
interface IncidentTypeFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  isEdit: boolean;
  editingIncidentType: IncidentTypeT | null;
  onSubmit: (values: IncidentTypeFormValues) => Promise<void>;
  submitErrors: SubmitErrorState;
}

export const IncidentTypeFormModal: React.FC<IncidentTypeFormModalProps> = ({
  isOpen,
  onClose,
  isEdit,
  editingIncidentType,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">{isEdit ? "Editar Tipo" : "Nuevo Tipo"}</h3>
            <p className="mt-0.5 text-sm text-slate-500">Configure el tipo de incidente de conducta</p>
          </div>
