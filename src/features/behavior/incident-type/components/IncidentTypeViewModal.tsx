...
interface State { data: IncidentTypeT | null; loading: boolean; error: string | null; }
type Action = { type: "loading" } | { type: "success"; data: IncidentTypeT } | { type: "error"; error: string };
...
interface IncidentTypeViewModalProps { isOpen: boolean; incidentTypeId: number | null; onClose: () => void; }

export const IncidentTypeViewModal: React.FC<IncidentTypeViewModalProps> = ({ isOpen, incidentTypeId, onClose }) => {
  const [state, dispatch] = useReducer(reducer, { data: null, loading: false, error: null });

  useEffect(() => {
    if (isOpen && incidentTypeId !== null) {
      dispatch({ type: "loading" });
      incidentTypeService.get({ id: incidentTypeId })
        .then((data) => dispatch({ type: "success", data }))
        .catch((err: Error) => dispatch({ type: "error", error: err.message }));
    }
  }, [isOpen, incidentTypeId]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Detalle de Tipo de Incidente</h3>
            <p className="mt-0.5 text-sm text-slate-500">Información del tipo de incidente</p>
          </div>
