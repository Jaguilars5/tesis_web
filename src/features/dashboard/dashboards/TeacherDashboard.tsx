    ...
    return (
      <div>
        <PageHeader
          ...
          description="Cargando tu información..."
        />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          {data.courses.length === 0 ? (
            <p className="mt-4 text-sm text-slate-400">Sin cursos asignados</p>
          ) : (
            <div className="mt-4 space-y-3">
              {data.courses.map((course) => (
                ...
                >
                  <div>
                    <p className="font-medium text-sm">{course.subject}</p>
                    <p className="text-xs text-slate-500">
                      {course.grade} · {course.students} estudiantes
                    </p>
                  </div>
                  {/* <div className="text-right">
                    <p className="text-xs text-slate-500">{course.nextClass}</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          {data.upcomingActivities.length === 0 ? (
            ...
            <div className="mt-4 space-y-3">
              {data.upcomingActivities.map((act, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between rounded-lg border border-slate-200 p-3"
                >
          ...
          <div className="mt-4 space-y-3">
            {data.alerts.map((alert, idx) => (
              ...
              >
                ...
                <Badge
                  variant={alert.level === "Alto" ? "destructive" : "secondary"}
                >
                  {alert.level}
                </Badge>
              </div>
