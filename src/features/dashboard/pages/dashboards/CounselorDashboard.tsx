  ...
  return (
    <div>
      <PageHeader
        ...
        description="Panel de intervención y seguimiento DECE"
      />
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="mt-4 space-y-3">
            {atRiskStudents.map((student, idx) => (
              <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-200 p-3">
                <div>
                  <p className="font-medium text-sm">{student.names} {student.apellidos}</p>
                  <p className="text-xs text-slate-500">{student.grado} - {student.seccion}</p>
                </div>
                <Badge variant="secondary">En riesgo</Badge>
              </div>
            ))}
          </div>
        </div>
