...
import { useAuthForm } from "../auth.controller";

interface FormValues { username: string; password: string; }

const getFieldError = (errors: Record<string, string>, touched: Record<string, boolean>, field: keyof FormValues): string | undefined =>
  ...
  return (
    <form className="space-y-4" onSubmit={formik.handleSubmit}>
      <div className="space-y-1.5">
        <CustomInput label="Usuario" id="username" name="username" type="text" placeholder="admin" autoComplete="username" disabled={isLoading}
          value={formik.values.username} className={{ ...inputClassname }} onBlur={formik.handleBlur} onChange={formik.handleChange}
          aria-invalid={!!(formik.touched.username && formik.errors.username)}
          error={getFieldError(formik.errors, formik.touched, "username")} />
      </div>
      <div className="space-y-1.5">
        <CustomInput label="Contraseña" id="password" name="password" type="password" placeholder="********" autoComplete="current-password" disabled={isLoading}
          value={formik.values.password} className={{ ...inputClassname }} onBlur={formik.handleBlur} onChange={formik.handleChange}
          error={getFieldError(formik.errors, formik.touched, "password")}
          aria-invalid={!!(formik.touched.password && formik.errors.password)} />
      </div>
      {authError && <p className="rounded-lg bg-red-50 border border-red-200 px-3 py-2 text-sm font-semibold text-danger">{authError}</p>}
