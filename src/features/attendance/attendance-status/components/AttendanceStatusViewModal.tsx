...
interface AttendanceStatusViewModalProps { isOpen: boolean; attendanceStatusId: number | null; onClose: () => void; }

export const AttendanceStatusViewModal: React.FC<AttendanceStatusViewModalProps> = ({ isOpen, attendanceStatusId, onClose }) => {
  const [state, dispatch] = useReducer(reducer, { data: null, loading: false, error: null });

  useEffect(() => {
    if (isOpen && attendanceStatusId !== null) {
      dispatch({ type: "loading" });
      attendanceStatusService.get({ id: attendanceStatusId })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Detalle de Estado de Asistencia</h3>
            <p className="mt-0.5 text-sm text-slate-500">Información del estado de asistencia</p>
          </div>
        ...
        <div className="space-y-5 p-6">
          {state.loading && (
            <div className="flex animate-pulse flex-col gap-4 py-4">
              {[1, 2].map((i) => (<div key={i} className="flex items-start gap-3"><div className="size-9 rounded-lg bg-slate-100" /><div className="flex-1 space-y-2"><div className="h-3 w-20 rounded bg-slate-100" /><div className="h-4 w-48 rounded bg-slate-100" /></div></div>))}
            </div>
          )}

          {state.error && (
            <div className="flex items-center gap-2.5 rounded-lg bg-red-50 p-4 text-sm text-red-600"><X className="size-4 shrink-0" />{state.error}</div>
          )}
