...
export const AttendanceTable = ({ attendances, isLoading, loadAttendances, onEdit, onView }: AttendanceTableProps) => {
  ...
  const fetchData = useCallback((overrides?: { page?: number; pageSize?: number; search?: string; ordering?: AttendanceOrderingT }) => {
    loadAttendances({
  ...
  const columns: TableColumnProps<AttendanceT>[] = [
    { key: "enrollment_name", label: "Estudiante", className: tableFirstColumnClassname, render: (s) => <span>{s.enrollment_name}</span> },
    { key: "attendance_date", label: "Fecha", className: tableColumnsClassname, render: (s) => <span>{fechaFormat(new Date(s.attendance_date), "DD/MM/YYYY") || s.attendance_date}</span> },
    { key: "attendance_status_name", label: "Estado", className: tableColumnsClassname },
    { key: "teacher_subject_section_name", label: "Clase", className: tableColumnsClassname, render: (s) => <span className="line-clamp-1 max-w-48 text-ellipsis">{s.teacher_subject_section_name}</span> },
  ...
  return (
    <div className="overflow-visible rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-slate-50/50 px-4 py-3">
        <SearchInput name="search" type="text" onChange={handleSearchChange} value={search} className="relative min-w-50 flex-1" placeholder="Filtrar asistencias..." />
      ...
      <CustomTable<AttendanceT>
        ...
        emptyMessage={hasSearched ? "No se encontraron asistencias con los filtros aplicados" : "No se encontraron asistencias"}
        actionsTitle="Acciones" className={tableClassname} loadingMessage="Cargando asistencias..."
        rowActions={(s) => (
      ...
      <Pagination page={page} pageSize={pageSize} totalItems={attendances.length} isLoading={isLoading} hasNextPage={hasNextPage}
