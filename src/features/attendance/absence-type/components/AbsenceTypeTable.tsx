...
export const AbsenceTypeTable = ({
  ...
  const [ordering, setOrdering] = useState<AbsenceTypeOrderingT>("name");
  const [hasSearched, setHasSearched] = useState(false);

  const debounceRef = useRef<ReturnType<typeof setTimeout>>(undefined);

  const fetchData = useCallback(
    (overrides?: { page?: number; pageSize?: number; search?: string; ordering?: AbsenceTypeOrderingT }) => {
      loadAbsenceTypes({
  ...
  return (
    <div className="overflow-visible rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-slate-50/50 px-4 py-3">
        <SearchInput name="search" type="text" onChange={handleSearchChange} value={search} className="relative min-w-50 flex-1" placeholder="Filtrar tipos de ausencia..." />
      ...
      <CustomTable<AbsenceTypeT>
        data={absenceTypes}
        columns={columns}
        isLoading={isLoading && absenceTypes.length === 0}
        emptyMessage={hasSearched ? "No se encontraron tipos de ausencia con los filtros aplicados" : "No se encontraron tipos de ausencia"}
        actionsTitle="Acciones"
        className={tableClassname}
        loadingMessage="Cargando tipos de ausencia..."
        rowActions={(s) => (
      ...
      <Pagination page={page} pageSize={pageSize} totalItems={absenceTypes.length} isLoading={isLoading} hasNextPage={hasNextPage}
