...
interface SubjectAcademicConfigDeleteModalProps {
  isOpen: boolean;
  config: SubjectAcademicConfigT | null;
  onClose: () => void;
  onConfirm: (id: number) => Promise<void>;
}

export const SubjectAcademicConfigDeleteModal = ({
  isOpen,
  config,
  onClose,
  onConfirm,
}: SubjectAcademicConfigDeleteModalProps) => {
          ...
          <div className="flex flex-col items-center gap-4 py-2 text-center">
            ...
            <div>
              <p className="text-sm text-slate-600">
                ¿Está seguro de desactivar esta configuracion?
              </p>
              <p className="mt-2 text-sm font-medium text-slate-900">
                {config.subject_name}
              </p>
              <p className="text-xs text-slate-500">
                {config.academic_grade_name} &middot; {config.weekly_hours}{" "}
                horas
              </p>
            </div>
          </div>
