...
interface SubjectOfferingDeleteModalProps {
  isOpen: boolean;
  entity: SubjectOfferingT | null;
  onClose: () => void;
  onSoftDelete: (params: SoftDeleteParamsT) => Promise<SoftDeleteResponseT>;
}

export const SubjectOfferingDeleteModal: React.FC<SubjectOfferingDeleteModalProps> = ({ isOpen, entity, onClose, onSoftDelete }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <h3 className="text-lg font-semibold text-slate-900">Desactivar Oferta</h3>
          ...
          {phase === "confirm" && (
            <div className="flex flex-col items-center gap-4 py-2 text-center">
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">{entity.subject_academic_config_name}</p>
                <p className="text-xs text-slate-500">{entity.school_year_name} &middot; {entity.section_name}</p>
