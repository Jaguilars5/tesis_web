...
const classScheduleSlice = createSlice({
  ...
  reducers: {
    ...
    scheduleCreated(state, action: PayloadAction<ClassScheduleT>) {
      state.classSchedules.unshift(action.payload);
      state.status = "succeeded";
    },
    scheduleUpdated(state, action: PayloadAction<ClassScheduleT>) {
    ...
    scheduleDeleted(state, action: PayloadAction<number>) {
