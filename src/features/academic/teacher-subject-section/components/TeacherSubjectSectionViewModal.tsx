...
interface State {
  data: TeacherSubjectSectionT | null;
  loading: boolean;
  error: string | null;
}
...
type Action =
  | { type: "loading" }
  | { type: "success"; data: TeacherSubjectSectionT }
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-500">
              Informacion de la asignacion docente-materia
            </p>
          </div>
          <button
            type="button"
          ...
          {state.data && (
            <div className="space-y-5">
              ...
              <DetailRow
                ...
                label="Año Escolar"
                value={state.data.subject_offering_school_year_name ?? "—"}
              />
              <DetailRow
                icon={<Hash className="size-4" />}
                label="Sección"
                value={state.data.subject_offering_section_name ?? "—"}
              />
              <DetailRow
                icon={<BookOpen className="size-4" />}
                label="Materia"
                value={state.data.subject_offering_subject_name ?? "—"}
              />
