...
interface SectionViewModalState {
  data: SectionT | null;
  loading: boolean;
  error: string | null;
}
type SectionViewModalAction =
  | { type: "loading" }
  | { type: "success"; data: SectionT }
  | { type: "error"; error: string };

const reducer = (
  state: SectionViewModalState,
  action: SectionViewModalAction,
): SectionViewModalState => {
  switch (action.type) {
    case "loading":
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-500">
              Información de la sección/paralelo
            </p>
          </div>
          <button
            type="button"
