import { BookOpen, FileText, X, Layers } from "lucide-react";
...
interface AcademicSubLevelViewModalState {
  data: AcademicSubLevelT | null;
  loading: boolean;
  error: string | null;
}

type AcademicSubLevelViewModalAction =
  | { type: "loading" }
  | { type: "success"; data: AcademicSubLevelT }
  | { type: "error"; error: string };

const reducer = (
  state: AcademicSubLevelViewModalState,
  action: AcademicSubLevelViewModalAction,
): AcademicSubLevelViewModalState => {
  switch (action.type) {
    case "loading":
...
interface AcademicSubLevelViewModalProps {
  isOpen: boolean;
  academicSubLevelId: number | null;
  onClose: () => void;
}

export const AcademicSubLevelViewModal: React.FC<
  AcademicSubLevelViewModalProps
> = ({ isOpen, academicSubLevelId, onClose }) => {
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-500">
              Información del subnivel académico
            </p>
          </div>
          <button
            type="button"
          ...
          {state.data && (
            <div className="space-y-5">
              ...
              <DetailRow
                ...
                label="Nivel"
                value={state.data.academic_level_name}
              />
