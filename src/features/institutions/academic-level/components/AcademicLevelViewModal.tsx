...
interface AcademicLevelViewModalState {
  data: AcademicLevelT | null;
  loading: boolean;
  error: string | null;
}
type AcademicLevelViewModalAction =
  | { type: "loading" }
  | { type: "success"; data: AcademicLevelT }
  | { type: "error"; error: string };
const reducer = (
  state: AcademicLevelViewModalState,
  action: AcademicLevelViewModalAction,
): AcademicLevelViewModalState => {
  switch (action.type) {
    case "loading":
...
interface AcademicLevelViewModalProps {
  isOpen: boolean;
  academicLevelId: number | null;
  onClose: () => void;
}
export const AcademicLevelViewModal: React.FC<AcademicLevelViewModalProps> = ({
  isOpen,
  academicLevelId,
  onClose,
}) => {
  ...
  useEffect(() => {
    if (isOpen && academicLevelId !== null) {
      dispatch({ type: "loading" });
      academicLevelService
        .get({ id: academicLevelId })
        .then((data) => dispatch({ type: "success", data }))
        .catch((err: Error) => dispatch({ type: "error", error: err.message }));
    }
  }, [isOpen, academicLevelId, dispatch, academicLevelService]);
