import { BookOpen, FileText, Layers, X } from "lucide-react";
...
interface AcademicGradeViewModalState {
  data: AcademicGradeT | null;
  loading: boolean;
  error: string | null;
}
type AcademicGradeViewModalAction =
  | { type: "loading" }
  | { type: "success"; data: AcademicGradeT }
  | { type: "error"; error: string };

const reducer = (
  state: AcademicGradeViewModalState,
  action: AcademicGradeViewModalAction,
): AcademicGradeViewModalState => {
  switch (action.type) {
    case "loading":
...
interface AcademicGradeViewModalProps {
  isOpen: boolean;
  academicGradeId: number | null;
  onClose: () => void;
}

export const AcademicGradeViewModal: React.FC<AcademicGradeViewModalProps> = ({
  isOpen,
  academicGradeId,
  onClose,
}) => {
          ...
          {state.data && (
            <div className="space-y-5">
              ...
              <DetailRow
                ...
                label="Subnivel"
                value={state.data.academic_sublevel_name || "—"}
              />
