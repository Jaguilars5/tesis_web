...
interface State { data: EvaluationBlockT | null; loading: boolean; error: string | null; }
type Action = { type: "loading" } | { type: "success"; data: EvaluationBlockT } | { type: "error"; error: string };
...
interface EvaluationBlockViewModalProps { isOpen: boolean; evaluationBlockId: number | null; onClose: () => void; }

export const EvaluationBlockViewModal: React.FC<EvaluationBlockViewModalProps> = ({ isOpen, evaluationBlockId, onClose }) => {
  const [state, dispatch] = useReducer(reducer, { data: null, loading: false, error: null });

  useEffect(() => {
    if (isOpen && evaluationBlockId !== null) {
      dispatch({ type: "loading" });
      evaluationBlockService.get({ id: evaluationBlockId })
        .then((data) => dispatch({ type: "success", data }))
        .catch((err: Error) => dispatch({ type: "error", error: err.message }));
    }
  }, [isOpen, evaluationBlockId]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div><h3 className="text-lg font-semibold text-slate-900">Detalle de Bloque</h3><p className="mt-0.5 text-sm text-slate-500">Información del bloque de evaluación</p></div>
        ...
        <div className="flex items-center justify-end border-t border-slate-200 px-6 py-4">
          <button type="button" onClick={onClose} className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cerrar</button>
        </div>
      </div>
    </div>
  );
