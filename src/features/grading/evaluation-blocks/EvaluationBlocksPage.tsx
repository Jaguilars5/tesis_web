...
export default function EvaluationBlocksPage() {
  ...
  const canCreate = hasPermission(
    permissions,
    EVALUATION_BLOCKS_PERMISSIONS.CREATE,
  );
  const canEdit = hasPermission(
    permissions,
    EVALUATION_BLOCKS_PERMISSIONS.UPDATE,
  );
  const canDelete = hasPermission(
    permissions,
    EVALUATION_BLOCKS_PERMISSIONS.DELETE,
  );
  ...
  const [deletingItem, setDeletingItem] = useState<EvaluationBlockT | null>(
    null,
  );
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);

  const openViewModal = useCallback((eb: EvaluationBlockT) => {
    setViewingItem(eb);
    setIsViewOpen(true);
  }, []);
  ...
  const openDeleteModal = useCallback((eb: EvaluationBlockT) => {
    setDeletingItem(eb);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Gestiona los bloques de evaluación
          </p>
        </div>
        {canCreate && (
          <button
      ...
      <EvaluationBlockTable
        evaluationBlocks={items}
        totalCount={totalCount}
        isLoading={isLoading}
        loadEvaluationBlocks={loadItems}
        onEdit={openModal}
      ...
      <EvaluationBlockFormModal
        ...
        onClose={closeModal}
        isEdit={isEdit}
        editingEvaluationBlock={editingItem}
      ...
      <EvaluationBlockViewModal
        isOpen={isViewOpen}
        evaluationBlockId={viewingItem?.id ?? null}
        onClose={closeViewModal}
      />

      <EvaluationBlockDeleteModal
        isOpen={isDeleteOpen}
        evaluationBlock={deletingItem}
