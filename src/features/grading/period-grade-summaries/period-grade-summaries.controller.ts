import { useAppDispatch, useAppSelector } from "@shared/redux/hooks";
import { useCallback, useState } from "react";
import { periodGradeSummaryService } from "./period-grade-summaries.service";
import {
  loadPending,
  loadSuccess,
  loadError,
  entityCreated,
  entityUpdated,
  entityDeleted,
  mutationError,
  selectPeriodGradeSummaries,
  selectPeriodGradeSummariesStatus,
  selectPeriodGradeSummariesError,
} from "./period-grade-summaries.slice";
import type {
  PeriodGradeSummaryCreateDataT,
  PeriodGradeSummaryCreateParamsT,
  PeriodGradeSummaryDeleteParamsT,
  PeriodGradeSummaryFormValues,
  PeriodGradeSummaryListParamsT,
  PeriodGradeSummaryT,
  PeriodGradeSummaryUpdateParamsT,
} from "./period-grade-summaries.types";
export type CreateRejectValue = {
  msg: string;
  data: Record<string, string> | null;
};
export type ValidationErrors = Record<string, string>;
export interface SubmitErrorState {
  general: string[];
  validation: ValidationErrors;
}
const toRejectValue = (e: unknown): CreateRejectValue => {
  const msg = e instanceof Error ? e.message : "Error desconocido";
  const cause =
    e instanceof Error ? (e as { cause?: unknown }).cause : undefined;
  const d = (cause as { response?: { data?: { data?: unknown } } })?.response
    ?.data;
  return { msg, data: (d?.data as Record<string, string> | null) ?? null };
};
export const usePeriodGradeSummariesController = () => {
  const dispatch = useAppDispatch();
  const periodGradeSummaries = useAppSelector(selectPeriodGradeSummaries);
  const status = useAppSelector(selectPeriodGradeSummariesStatus);
  const error = useAppSelector(selectPeriodGradeSummariesError);
  const loadPeriodGradeSummaries = useCallback(
    async (p?: PeriodGradeSummaryListParamsT) => {
      dispatch(loadPending());
      try {
        const items = await periodGradeSummaryService.list(
          p ?? { page: 1, pageSize: 100 },
        );
        dispatch(loadSuccess(items));
      } catch (err) {
        dispatch(loadError(err instanceof Error ? err.message : "Error"));
      }
    },
    [dispatch],
  );
  const create = useCallback(
    async (
      d: PeriodGradeSummaryCreateParamsT,
    ): Promise<PeriodGradeSummaryT> => {
      try {
        const c = await periodGradeSummaryService.create(d);
        dispatch(entityCreated(c));
        return c;
      } catch (err) {
        const rv = toRejectValue(err);
        dispatch(mutationError(rv.msg));
        throw rv;
      }
    },
    [dispatch],
  );
  const update = useCallback(
    async (
      p: PeriodGradeSummaryUpdateParamsT,
    ): Promise<PeriodGradeSummaryT> => {
      try {
        const u = await periodGradeSummaryService.update(p);
        dispatch(entityUpdated(u));
        return u;
      } catch (err) {
        const rv = toRejectValue(err);
        dispatch(mutationError(rv.msg));
        throw rv;
      }
    },
    [dispatch],
  );
  const remove = useCallback(
    async (id: PeriodGradeSummaryDeleteParamsT): Promise<void> => {
      try {
        const { id: did } = await periodGradeSummaryService.softDelete(id);
        dispatch(entityDeleted(did));
      } catch (err) {
        dispatch(mutationError(err instanceof Error ? err.message : "Error"));
      }
    },
    [dispatch],
  );
  return {
    periodGradeSummaries,
    isLoading: status === "loading",
    error,
    loadPeriodGradeSummaries,
    createPeriodGradeSummary: create,
    updatePeriodGradeSummary: update,
    deletePeriodGradeSummary: remove,
  };
};
export type PGSControllerT = ReturnType<
  typeof usePeriodGradeSummariesController
>;
const extractError = (e: unknown) => {
  if (
    e &&
    typeof e === "object" &&
    "msg" in e &&
    typeof (e as { msg?: unknown }).msg === "string"
  ) {
    const obj = e as CreateRejectValue;
    const fes: ValidationErrors = {};
    const gen: string[] = [];
    if (obj.data && typeof obj.data === "object") {
      for (const [k, v] of Object.entries(obj.data)) {
        if (typeof v === "string") fes[k] = v;
        else gen.push(`${k}: ${v}`);
      }
    }
    if (gen.length === 0 && Object.keys(fes).length === 0) gen.push(obj.msg);
    return { msg: obj.msg, general: gen, validation: fes };
  }
  const msg = e instanceof Error ? e.message : "Error desconocido";
  return { msg, general: [msg], validation: {} };
};
const uwCreate = async (
  d: PeriodGradeSummaryCreateDataT,
  create: PGSControllerT["createPeriodGradeSummary"],
) => {
  try {
    return {
      ok: true as const,
      result: await create(d),
      errors: { general: [], validation: {} } as SubmitErrorState,
    };
  } catch (e) {
    const p = extractError(e);
    return {
      ok: false as const,
      result: null,
      errors: {
        general: p.general,
        validation: p.validation,
      } as SubmitErrorState,
    };
  }
};
const uwUpdate = async (
  p: PeriodGradeSummaryUpdateParamsT,
  update: PGSControllerT["updatePeriodGradeSummary"],
) => {
  try {
    return {
      ok: true as const,
      result: await update(p),
      errors: { general: [], validation: {} } as SubmitErrorState,
    };
  } catch (e) {
    const p2 = extractError(e);
    return {
      ok: false as const,
      result: null,
      errors: {
        general: p2.general,
        validation: p2.validation,
      } as SubmitErrorState,
    };
  }
};
interface UseFormArgs {
  create: PGSControllerT["createPeriodGradeSummary"];
  update: PGSControllerT["updatePeriodGradeSummary"];
}
export const usePeriodGradeSummariesForm = ({
  create,
  update,
}: UseFormArgs) => {
  const [isOpen, setIsOpen] = useState(false);
  const [editing, setEditing] = useState<PeriodGradeSummaryT | null>(null);
  const [submitErrors, setSubmitErrors] = useState<SubmitErrorState>({
    general: [],
    validation: {},
  });
  const isEdit = !!editing;
  const openModal = useCallback((s?: PeriodGradeSummaryT) => {
    setEditing(s ?? null);
    setSubmitErrors({ general: [], validation: {} });
    setIsOpen(true);
  }, []);
  const closeModal = useCallback(() => {
    setIsOpen(false);
    setEditing(null);
    setSubmitErrors({ general: [], validation: {} });
  }, []);
  const handleSubmit = useCallback(
    async (values: PeriodGradeSummaryFormValues) => {
      setSubmitErrors({ general: [], validation: {} });
      if (editing) {
        const r = await uwUpdate({ id: editing.id, data: values }, update);
        if (r.ok) {
          closeModal();
          return;
        }
        setSubmitErrors(r.errors);
      } else {
        const {
          formative_avg,
          summative_avg,
          final_avg_truncated,
          requires_recovery,
          enrollment,
          subject_offering,
          academic_period,
          qualitative_scale,
          promotion_status,
        } = values;
        const r = await uwCreate(
          {
            formative_avg,
            summative_avg,
            final_avg_truncated,
            requires_recovery,
            enrollment,
            subject_offering,
            academic_period,
            qualitative_scale,
            promotion_status,
          },
          create,
        );
        if (r.ok) {
          closeModal();
          return;
        }
        setSubmitErrors(r.errors);
      }
    },
    [editing, create, update, closeModal],
  );
  return {
    isOpen,
    isEdit,
    editingPeriodGradeSummary: editing,
    submitErrors,
    openModal,
    closeModal,
    handleSubmit,
  };
};
