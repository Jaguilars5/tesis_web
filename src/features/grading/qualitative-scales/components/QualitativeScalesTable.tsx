...
const O: { label: string; value: QualitativeScaleOrderingT }[] = [
...
type Props = {
  ...
  isLoading: boolean;
  loadQualitativeScales: (p?: QualitativeScaleListParamsT) => void;
  onEdit: (s: QualitativeScaleT) => void;
  onView: (s: QualitativeScaleT) => void;
  onDelete: (s: QualitativeScaleT) => void;
};
export const QualitativeScalesTable = ({
  qualitativeScales,
  isLoading,
  ...
  const hOrder = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      const n = e.target.value as QualitativeScaleOrderingT;
      setOrdering(n);
      setPage(1);
  ...
  const hnp = qualitativeScales.length >= pageSize;
  const cols: TableColumnProps<QualitativeScaleT>[] = [
