...
interface State {
  data: QualitativeScaleSublevelT | null;
  loading: boolean;
  error: string | null;
}
...
type Action =
  | { type: "loading" }
  | { type: "success"; data: QualitativeScaleSublevelT }
...
interface Props {
  isOpen: boolean;
  qualitativeScaleSublevelId: number | null;
  onClose: () => void;
}

export const QualitativeScaleSublevelViewModal: React.FC<Props> = ({
  isOpen,
  qualitativeScaleSublevelId,
  onClose,
}) => {
  ...
  useEffect(() => {
    if (isOpen && qualitativeScaleSublevelId !== null) {
      dispatch({ type: "loading" });
      qualitativeScaleSublevelService
        .get({ id: qualitativeScaleSublevelId })
        .then((data) => dispatch({ type: "success", data }))
        .catch((err: Error) =>
          dispatch({ type: "error", error: err.message }),
        );
    }
  }, [isOpen, qualitativeScaleSublevelId]);
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-500">
              Información de la escala cualitativa por subnivel
            </p>
          </div>
          <button
            type="button"
          ...
          {state.data && (
            <div className="space-y-5">
              <DetailRow
                ...
                label="Escala"
                value={state.data.scale_name}
              />
              <DetailRow
                icon={<BookOpen className="size-4" />}
                label="Subnivel"
                value={state.data.sublevel_name}
              />
            </div>
          )}
        </div>
