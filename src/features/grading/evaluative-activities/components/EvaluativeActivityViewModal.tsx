...
interface State { data: EvaluativeActivityT | null; loading: boolean; error: string | null; }
type Action = { type: "loading" } | { type: "success"; data: EvaluativeActivityT } | { type: "error"; error: string };
...
interface EvaluativeActivityViewModalProps { isOpen: boolean; evaluativeActivityId: number | null; onClose: () => void; }

export const EvaluativeActivityViewModal: React.FC<EvaluativeActivityViewModalProps> = ({ isOpen, evaluativeActivityId, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Detalle de Actividad</h3>
            <p className="mt-0.5 text-sm text-slate-500">Información de la actividad evaluativa</p>
          </div>
        ...
        <div className="space-y-5 p-6">
          {state.loading && (<div className="flex animate-pulse flex-col gap-4 py-4">{[1, 2, 3].map((i) => (<div key={i} className="flex items-start gap-3"><div className="size-9 rounded-lg bg-slate-100" /><div className="flex-1 space-y-2"><div className="h-3 w-20 rounded bg-slate-100" /><div className="h-4 w-48 rounded bg-slate-100" /></div></div>))}</div>)}
          ...
          {state.data && (
            <div className="space-y-5">
              <DetailRow icon={<BookOpen className="size-4" />} label="Título" value={state.data.title} />
              <DetailRow icon={<BookOpen className="size-4" />} label="Clase" value={state.data.teacher_subject_section_name} />
              <DetailRow icon={<BookOpen className="size-4" />} label="Bloque" value={state.data.block_component_name} />
              <DetailRow icon={<Calendar className="size-4" />} label="Vencimiento" value={state.data.due_date} />
              <DetailRow icon={<BookOpen className="size-4" />} label="Punt. Máx." value={state.data.max_score} />
