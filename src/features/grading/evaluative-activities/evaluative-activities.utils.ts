...
export const evaluativeActivitySchema = Yup.object({
  ...
  teacher_subject_section: Yup.number().required("La sección docente-materia es obligatoria"),
  ...
  block_component: Yup.number().min(1).required("El bloque de evaluación es obligatorio"),
