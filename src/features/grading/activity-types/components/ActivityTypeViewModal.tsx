...
interface State {
  data: ActivityTypeT | null;
  loading: boolean;
  error: string | null;
}
type Action =
  | { type: "loading" }
  | { type: "success"; data: ActivityTypeT }
...
interface ActivityTypeViewModalProps {
  isOpen: boolean;
  activityTypeId: number | null;
  onClose: () => void;
}

export const ActivityTypeViewModal: React.FC<ActivityTypeViewModalProps> = ({
  isOpen,
  activityTypeId,
  onClose,
}) => {
  ...
  useEffect(() => {
    if (isOpen && activityTypeId !== null) {
      dispatch({ type: "loading" });
      activityTypeService
        .get({ id: activityTypeId })
        .then((data) => dispatch({ type: "success", data }))
        .catch((err: Error) => dispatch({ type: "error", error: err.message }));
    }
  }, [isOpen, activityTypeId]);
