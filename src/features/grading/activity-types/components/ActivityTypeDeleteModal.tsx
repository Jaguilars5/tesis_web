...
interface ActivityTypeDeleteModalProps {
  isOpen: boolean;
  activityType: ActivityTypeT | null;
  onClose: () => void;
  onSoftDelete: (
...
export const ActivityTypeDeleteModal: React.FC<
  ActivityTypeDeleteModalProps
> = ({ isOpen, activityType, onClose, onSoftDelete }) => {
  const {
    phase,
    message,
    affectedRecords,
          ...
          {phase === "confirm" && (
            <div className="flex flex-col items-center gap-4 py-2 text-center">
              ...
              <div className="space-y-2">
                ...
                {activityType.code && (
                  <p className="text-xs text-slate-500">{activityType.code}</p>
                )}
                {message && (
