...
interface State { data: BlockComponentT | null; loading: boolean; error: string | null; }
type Action = { type: "loading" } | { type: "success"; data: BlockComponentT } | { type: "error"; error: string };
...
interface BlockComponentViewModalProps { isOpen: boolean; blockComponentId: number | null; onClose: () => void; }

export const BlockComponentViewModal: React.FC<BlockComponentViewModalProps> = ({ isOpen, blockComponentId, onClose }) => {
  const [state, dispatch] = useReducer(reducer, { data: null, loading: false, error: null });

  useEffect(() => {
    if (isOpen && blockComponentId !== null) {
      dispatch({ type: "loading" });
      blockComponentService.get({ id: blockComponentId })
        .then((data) => dispatch({ type: "success", data }))
        .catch((err: Error) => dispatch({ type: "error", error: err.message }));
    }
  }, [isOpen, blockComponentId]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg animate-slide-up overflow-hidden rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Detalle de Componente de Bloque</h3>
            <p className="mt-0.5 text-sm text-slate-500">Información del componente de bloque</p>
          </div>
        ...
        <div className="space-y-5 p-6">
          {state.loading && (
            <div className="flex animate-pulse flex-col gap-4 py-4">
              {[1, 2, 3].map((i) => (<div key={i} className="flex items-start gap-3"><div className="size-9 rounded-lg bg-slate-100" /><div className="flex-1 space-y-2"><div className="h-3 w-20 rounded bg-slate-100" /><div className="h-4 w-48 rounded bg-slate-100" /></div></div>))}
            </div>
          )}
          ...
          {state.data && (
            <div className="space-y-5">
              ...
              <DetailRow icon={<Percent className="size-4" />} label="Ponderación" value={`${state.data.internal_weight}%`} />
              <DetailRow icon={<BookOpen className="size-4" />} label="Bloque" value={state.data.evaluation_block_name} />
